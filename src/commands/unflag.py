...
class UnflagCommand(commands.Cog):
    ...
    async def unflag(
        ...
        success = unflag_user(interaction.guild_id, user_id)
        
        if not success:
            embed = create_error_embed(
                title="Not Flagged",
                description="This user is not currently on the flagged list."
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        embed = create_success_embed(
            ...
            description=f"{user_display} has been removed from the flagged list.\n\nThey can now access links again."
        )
...
async def setup(bot: commands.Bot):
    await bot.add_cog(UnflagCommand(bot))
